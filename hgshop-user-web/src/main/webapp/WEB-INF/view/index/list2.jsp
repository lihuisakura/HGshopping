...
function goPage(page){
	var id= $("[name='catId']").val();
	query(id,page);
}
function search(){
	var key=$("[name=key]").val();
	location="/goods/search?key="+key;
}
</script>
<style type="text/css">
 a {
...
<body id="top">
			...
			<div class="header-box">
				...
				<ul class="header-right">
					...
					<c:if test="${user!=null}">
						...
						<a href="/user/logout"  class="red">退出登录</a>
					</c:if>
					</li> 
					<li class="shu"></li>
	...
	<div class="focus">
		...
		<!-- 中间商品显示 -->
		<div style="clear:both;position:absolute;top:63px;left:250px;width:1400px;" >
			<div class="row"> 
					...
					<div class="row">
						<c:forEach items="${spuInfo.list}" var="spu">
							<div class="col-md-3" style="margin-top:30px" >
								<div><img src="/pic/${spu.smallPic}" onclick="location='/goods/detail?id=${spu.id}'" width="160px" height="160px"> </div>
				...
				<div class="col-md-8">
					<div class="row">
						<c:forEach items="${goodsList.list}" var="goods">
							<div class="col-md-3" style="margin-top:30px" >
								<div><img src="/pic/${goods.smallPic}" onclick="location='/goods/detail?id=${goods.id}'" width="160px" height="160px"> </div>
								<div><a href="/goods/detail?id=${goods.id}">${goods.goodsName}</a></div>
								<div><a href="/goods/detail?id=${goods.id}">${goods.caption}</a></div>
							</div>
						</c:forEach>
					</div>
		...
		<nav aria-label="Page navigation example" style="margin-top:50px">
		  <ul class="pagination justify-content-center">
		    ...
		    <c:forEach begin="1" end="${goodsList.pages}" var="page">
		      <li class="page-item"><a class="page-link" href="#" onclick="goPage(${page})">${page}</a></li>
		   </c:forEach>
		    <li class="page-item">
		      <a class="page-link" href="#"   onclick="goPage(${goodsList.pages})">尾页</a>
		    </li>
		  </ul>
		</nav>
